...
class RiskParameters:
    """Weekly ROI focused risk parameters"""
    stop_loss_pct: float = 0.015            # 1.5% stop-loss for weekly trades
    profit_target_pct: float = 0.05          # 5% profit target for weekly compounding
    time_stop_days: int = 7                 # 7 days maximum for weekly cycles
    confidence_threshold: float = 0.5        # Lower threshold for weekly opportunities
    
    # Weekly ROI trailing stop parameters
    trailing_stop_pct: float = 0.02          # 2% trailing stop (tighter for weekly)
    breakout_profit_target: float = 0.08     # 8% target for weekly breakouts
    momentum_extension_days: int = 7        # Maximum 7 days even for strong momentum
