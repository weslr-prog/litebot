...
class RiskPerTradeConfig:
    """Configuration for weekly ROI trading position sizing"""
    # WEEKLY ROI TRADING PARAMETERS
    risk_per_trade_pct: float = 0.015       # 1.5% risk per trade for weekly cycles
    max_position_pct: float = 0.10          # 10% max position for weekly diversification
    min_position_pct: float = 0.02          # 2% minimum position for weekly diversification
    
    # Weekly ROI stop-loss parameters  
    default_stop_loss_pct: float = 0.015    # 1.5% default stop for weekly trades
    min_stop_loss_pct: float = 0.01         # 1% minimum stop
    max_stop_loss_pct: float = 0.025        # 2.5% maximum stop for weekly cycles
    
    # Weekly ROI position limits
    max_concurrent_positions: int = 20      # 15-25 position range for weekly diversification
    cash_buffer_pct: float = 0.05           # 5% cash buffer
    
    # Enhanced for weekly momentum bursts
    breakout_multiplier: float = 1.2        # 1.2x sizing for weekly breakouts
    momentum_threshold: float = 0.08        # 8% momentum for weekly breakout sizing
