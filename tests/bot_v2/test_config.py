...
class TestShortCycleConfig:
    ...
    def test_default_config(self):
        ...
        assert config.daily_pool_percent == 0.40
        assert config.max_position_dollars == 150.0
        assert config.confidence_threshold == 0.35
        assert config.friday_loser_threshold == -0.03
        assert config.fast_exit_threshold_pct == 0.015
        
        # Verify derived values calculated
        assert config.daily_pool_dollars == 400.0  # 1000 * 0.40
