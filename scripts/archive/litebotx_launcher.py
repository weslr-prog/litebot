...
def create_trading_config(profile="balanced"):
    ...
    profiles = {
        "conservative": {
            ...
            'confidence_threshold': 0.10,  # INCREASED: More selective after drawdown (was 0.08)
    ...
    return ShortCycleConfig(
        ...
        max_position_dollars=settings['max_position_dollars'],  # NEW: Pass through hard cap
        max_loss_per_trade_dollars=settings['max_loss_per_trade_dollars'],  # NEW: Pass through max loss
...
def run_trading_system(profile):
    ...
    try:
        # Import intraday config
        from config import ENABLE_INTRADAY_ANALYSIS, MAX_INTRADAY_ANALYSES_PER_DAY
        # Initialize trader with PreFilter intraday enhancement for real trading
        trader = ShortCycleTrader(
            config,
            enable_intraday_analysis=ENABLE_INTRADAY_ANALYSIS if profile == "aggressive" else False,
            max_intraday_analyses_per_day=MAX_INTRADAY_ANALYSES_PER_DAY if profile == "aggressive" else 0
        )
