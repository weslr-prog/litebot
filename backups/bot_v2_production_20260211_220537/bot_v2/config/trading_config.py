...
class ShortCycleConfig:
    ...
    # Portfolio parameters (Jan 24, 2026: 75% Capital Utilization Strategy)
    ...
    # Profit targets (WEEKLY SWING - Feb 11, 2026)
    # Target 4% per trade to reach ~5% weekly goal
    profit_target_pct: float = 0.04  # 4% profit target (unified - was conflicting 2% and 4%)
    ...
    # Default targets (Feb 11, 2026: UNIFIED for weekly swing strategy)
    # Single source of truth - these values are used everywhere
    profit_target_pct: float = 0.04  # 4% profit target
    stop_loss_pct: float = 0.02  # 2% stop loss (tight - cut losers fast)
