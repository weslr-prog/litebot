    ...
    def process_exits(self, current_prices: dict):
        ...
        for position in self.positions:
            ...
            if self.stop_manager.should_fast_exit(position, current_price):
                ...
                position.exit_timestamp = dt.datetime.now(dt.timezone.utc)
                position.exit_date = position.exit_timestamp.date()
                position.realized_pnl = (position.exit_price - position.entry_price) * position.position_size_shares
                exits_processed += 1
                continue
