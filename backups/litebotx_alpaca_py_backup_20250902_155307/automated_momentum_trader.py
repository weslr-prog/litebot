...
class AutomatedMomentumTrader:
    ...
    def setup_schedule(self):
        ...
        logging.info("📅 Setting up automated trading schedule...")
        
        # Pre-market analysis (7:30 AM ET) - prepare for the day
        schedule.every().day.at("07:30").do(self.portfolio_summary).tag('summary')
        
        # Market open momentum check (9:45 AM ET) - 15 minutes after open
        schedule.every().day.at("09:45").do(self.execute_momentum_cycle).tag('trading')
        
        # Mid-morning rebalance (11:00 AM ET)
        schedule.every().day.at("11:00").do(self.execute_momentum_cycle).tag('trading')
        
        # Noon momentum check (12:00 PM ET)
        schedule.every().day.at("12:00").do(self.execute_momentum_cycle).tag('trading')
        
        # Lunch time check (1:00 PM ET)
        schedule.every().day.at("13:00").do(self.execute_momentum_cycle).tag('trading')
        
        # Afternoon momentum check (2:00 PM ET)
        schedule.every().day.at("14:00").do(self.execute_momentum_cycle).tag('trading')
        
        # End of day rebalance (3:30 PM ET) - 30 minutes before close
        schedule.every().day.at("15:30").do(self.execute_momentum_cycle).tag('trading')
        ...
        logging.info("   🚀 09:45 ET - Market Open Momentum Check")
        logging.info("   🔄 11:00 ET - Mid-Morning Rebalance")
        logging.info("   🍽️ 12:00 ET - Noon Momentum Check")
        logging.info("   🕐 13:00 ET - Lunch Time Check")
        logging.info("   📈 14:00 ET - Afternoon Momentum Check")
