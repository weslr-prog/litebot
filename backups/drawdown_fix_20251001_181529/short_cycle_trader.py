...
class ShortCycleConfig:
    ...
    confidence_threshold: float = 0.055  # Optimized for efficiency & profitability  # Adjusted to 7% per live trading directive
...
class ShortCyclePosition:
    ...
    def should_smart_exit(self, current_date: dt.date, current_price: float, current_time: dt.datetime = None) -> tuple[bool, str]:
        ...
        # 6. Stop loss override: always exit if down significantly
        if pnl_pct < -0.03:  # Down more than 3%
...
class ShortCycleTrader:
    def _generate_portfolio_summary(self):
        ...
        try:
            ...
            daily_pnl = sum(p.realized_pnl or 0 for p in self.positions if p.exit_reason and 
                          p.exit_timestamp and p.exit_timestamp.date() == dt.date.today())
    ...
    def _process_existing_positions(self):
        ...
        for position in self.positions:
            ...
            try:
                ...
                # Check for smart D+1 exit (replaces simple forced exit)
    ...
    def _has_same_day_activity(self, symbol: str) -> bool:
        """
        Check if there's been any recent trading activity for this symbol.
        Prevents intraday round trips and rapid re-entry that violate swing trading approach.
        
        Returns True if the symbol has:
        1. Been entered today (same-day entry)
        2. Been exited today (same-day exit)  
        3. Been entered in the last 12 hours (prevents after-hours -> next day trades)
        """
        today = dt.date.today()
        now = dt.datetime.now()
        twelve_hours_ago = now - dt.timedelta(hours=12)
        
        # Check for same-day entries (positions entered today)
        for position in self.positions:
            if position.symbol == symbol and position.entry_date == today:
                self.logger.debug(f"🔄 {symbol}: Same-day entry detected (entered today)")
                return True
        
        # Check for same-day exits (positions exited today)
        for position in self.positions:
            if (position.symbol == symbol and 
                position.exit_timestamp and 
                position.exit_timestamp.date() == today):
                self.logger.debug(f"🔄 {symbol}: Same-day exit detected (exited today)")
                return True
        ...
        for position in self.positions:
            if (position.symbol == symbol and 
                ...
                self.logger.debug(f"🔄 {symbol}: Recent entry detected ({hours_ago:.1f} hours ago)")
                return True
