...
class AutomatedMomentumTraderV2:
    ...
    def __init__(self, symbols: List[str] = None, alpha_vantage_key: str = None, use_enhanced_strategy: bool = True):
        ...
        # Initialize risk-per-trade position sizer with AGGRESSIVE SWING TRADING parameters
        risk_per_trade_config = RiskPerTradeConfig(
            risk_per_trade_pct=0.02,      # 2% risk per trade (AGGRESSIVE - was 0.005)
            max_position_pct=0.20,        # 20% max position (AGGRESSIVE - was 0.15)
            min_position_pct=0.05,        # 5% min position (CONCENTRATED - was 0.01)
            max_concurrent_positions=5,   # 5 concentrated positions (was 10)
            default_stop_loss_pct=0.025,  # 2.5% stop-loss (TIGHTER - was 0.03)
            min_stop_loss_pct=0.015,      # 1.5% min stop
            max_stop_loss_pct=0.04        # 4% max stop
        )
        ...
        # Initialize enhanced momentum calculator with AGGRESSIVE SWING TRADING parameters
    ...
    def execute_enhanced_momentum_cycle(self):
        ...
        try:
            ...
            # 2. Detect market regime and adjust strategy accordingly
            logging.info("🌐 Detecting market regime...")
            current_regime, regime_confidence = self.regime_controller.detect_market_regime(market_data)
            
            regime_summary = self.regime_controller.get_regime_summary()
            logging.info(f"📊 Current Regime: {current_regime} (confidence: {regime_confidence:.1%})")
            ...
            max_exposure = self.regime_controller.get_regime_adjusted_exposure(self.portfolio_value)
            
            # 3. Generate momentum signals with regime-adjusted parameters
            # 3. Generate momentum signals with regime-adjusted parameters
            logging.info("🎯 Generating regime-aware momentum signals...")
            
            # Get regime-adjusted lookback periods
            base_lookback = getattr(self.momentum_strategy, 'lookback_1m', 21)
            adjusted_lookback = self.regime_controller.get_regime_adjusted_lookback(base_lookback)
            
            # 3. Generate enhanced risk-adjusted momentum signals
            logging.info("🎯 Generating enhanced risk-adjusted momentum signals...")
            
            # Use enhanced momentum calculator for superior stock selection
            enhanced_momentum_signals = self.enhanced_momentum.rank_stocks_by_momentum_quality(
                market_data,
                regime=current_regime,
                max_selections=20
            )
            
            if not enhanced_momentum_signals:
                logging.info("📊 No enhanced momentum signals generated for current regime")
                return
                
            # Convert to standard signal format with risk-adjusted scores
            momentum_signals = []
            for signal in enhanced_momentum_signals:
                momentum_signals.append({
                    'symbol': signal['symbol'],
                    'momentum_score': signal['momentum_score'],
                    'confidence': min(1.0, max(0.0, (signal['momentum_score'] + 1) / 3)),  # Convert to 0-1 confidence
                    'quality': signal['quality'],
                    'regime_optimized': True
                })
                
            logging.info(f"📊 Generated {len(momentum_signals)} enhanced momentum signals")
            ...
            regime_filtered_signals = self.regime_controller.filter_signals_by_regime(momentum_signals)
            ...
            # 5. Apply risk-per-trade position sizing (superior to portfolio % method)
            logging.info("🎯 Applying risk-per-trade position sizing...")
            risk_adjusted_signals = self.risk_per_trade_sizer.calculate_positions_for_signals(
                    ...
                    if side == 'buy':
                        try:
                            ...
                            if symbol in latest_data and not latest_data[symbol].empty:
                                ...
                                self.position_entry_dates[symbol] = entry_date
                                logging.info(f"📝 Tracking entry: {symbol} @ ${entry_price:.2f} on {entry_date}")
                        except Exception as e:
    ...
    def check_exit_conditions(self):
        ...
        try:
            ...
            for symbol, position in positions.items():
                ...
                # If we don't have entry data, use current data as baseline
                if not entry_price:
                    self.position_entry_prices[symbol] = current_price
                    self.position_entry_dates[symbol] = current_date
                    logging.info(f"📝 Tracking new position: {symbol} @ ${current_price:.2f}")
                    continue
                
                # Get dynamic risk parameters from adaptive manager
                current_params = self.adaptive_risk.get_current_parameters()
                
                # Calculate stop-loss and profit target prices using adaptive parameters
                stop_loss_price = entry_price * (1 - current_params.stop_loss_pct)
                profit_target_price = entry_price * (1 + current_params.profit_target_pct)
                
                # Check time-based exit using adaptive time stop
                max_hold_days = current_params.time_stop_days
                if entry_date:
                    try:
                        ...
                        trading_days_held = days_held * (5/7)  # Rough approximation
                        
                        should_time_exit = trading_days_held >= max_hold_days
                    except:
                ...
                if current_price <= stop_loss_price:
                    exit_reason = f"STOP-LOSS (${current_price:.2f} <= ${stop_loss_price:.2f})"
                elif current_price >= profit_target_price:
                    exit_reason = f"PROFIT-TARGET (${current_price:.2f} >= ${profit_target_price:.2f})"
                elif should_time_exit:
                    exit_reason = f"TIME-STOP ({trading_days_held:.1f}/{max_hold_days} days)"
                ...
                if exit_reason:
                    ...
                    logging.info(f"🚨 EXIT SIGNAL: {symbol} - {exit_reason}")
                else:
                    logging.info(f"✅ {symbol}: ${current_price:.2f} (stop: ${stop_loss_price:.2f}, target: ${profit_target_price:.2f})")
            
            # Execute exit trades
            if exit_trades:
                logging.info(f"📤 Executing {len(exit_trades)} exit trades...")
                for trade in exit_trades:
                    ...
                    try:
                        ...
                        if order_result:
                            logging.info(f"✅ EXIT: {symbol} {shares} shares - {reason} - Order: {order_result['order_id']}")
                            
                            # Record trade for adaptive learning
                            entry_date_for_trade = self.position_entry_dates.get(symbol, current_date)
                            ...
                            logging.error(f"❌ EXIT FAILED: {symbol}")
                ...
                logging.info("✅ No exit conditions triggered")
        ...
        try:
            ...
            if is_premarket and market_data:
                ...
                if positions:
                    ...
                    for symbol in list(positions.keys())[:5]:  # Top 5 positions
