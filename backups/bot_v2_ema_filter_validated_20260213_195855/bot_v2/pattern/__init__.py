...
class PatternRecognizer:
    ...
    def detect_peak(self, price_history: List[float], current_price: float, 
                    entry_price: float) -> Tuple[bool, str]:
        """
        Detect if price has peaked (momentum slowing or pullback starting).
        
        Args:
            price_history: List of prices since entry (chronological, at least 5 required)
            current_price: Current stock price
            entry_price: Entry price
            
        Returns:
            Tuple of (peak_detected: bool, reason: str)
        """
        try:
            # Need at least 5 price points to detect peak
            if not price_history or len(price_history) < 5:
                return False, "INSUFFICIENT_DATA"
            
            # Calculate rate of change for recent vs earlier periods
            # Recent = last 3 price changes
            # Earlier = previous 3 price changes
            if len(price_history) >= 6:
                ...
                # Peak signal 1: Momentum slowing significantly
                if earlier_momentum > 0 and recent_momentum < (earlier_momentum * 0.5):
                    pct_slow = ((earlier_momentum - recent_momentum) / earlier_momentum) * 100
                    return True, f"MOMENTUM_SLOWING_{pct_slow:.0f}PCT"
            
            # Peak signal 2: Pullback from recent high
            recent_high = max(price_history[-3:])
            pullback_pct = ((recent_high - current_price) / recent_high)
            
            if pullback_pct > 0.005:  # 0.5% pullback from recent high
                return True, f"PULLBACK_{pullback_pct*100:.1f}PCT"
            
            # Peak signal 3: Lower high pattern (failed to break previous high)
            if len(price_history) >= 8:
                # Compare last 4 prices to previous 4
                recent_max = max(price_history[-4:])
                earlier_max = max(price_history[-8:-4])
                
                if recent_max < earlier_max and current_price < recent_max:
                    return True, "LOWER_HIGH_PATTERN"
            
            # Peak signal 4: Profit taken and reversing (for strong gains)
            pnl_pct = (current_price - entry_price) / entry_price
            if pnl_pct >= 0.02:  # Up 2%+
                # Check if last 2 price moves were down
                if len(price_history) >= 3:
                    last_two_moves = [price_history[i] - price_history[i-1] 
                                     for i in range(-2, 0)]
                    if all(move < 0 for move in last_two_moves):
                        return True, f"REVERSAL_AT_{pnl_pct*100:.1f}PCT"
            
            return False, "NO_PEAK_DETECTED"
            
        except Exception as e:
            logger.error(f"Error detecting peak: {e}")
            return False, "ERROR"
