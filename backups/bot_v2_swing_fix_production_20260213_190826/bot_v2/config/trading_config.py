...
class ShortCycleConfig:
    ...
    # Trailing Stop Parameters (SWING FIX Feb 13: UNIFIED with SmartExitManager)
    # Single activation threshold, single trail distance - no more conflicting paths
    # BACKTEST RESULT: Trailing stop DISABLED — clips winners at +2.15% avg instead of +6% target
    # Binary model (4% stop / 6% target / 7-day time stop) yields R=1.35, best expectancy
    enable_trailing_stops: bool = False  # DISABLED: trailing is net drag on 2-5 day swing holds
    trailing_trigger_pct: float = 0.99  # Effectively disabled (activate only at +99%)
    trailing_distance_pct: float = 0.99  # Effectively disabled
