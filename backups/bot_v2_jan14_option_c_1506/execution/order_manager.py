...
class AIOrderManager:
    ...
    def _calculate_exit_date(self, entry_date: dt.date, symbol: str = None) -> dt.date:
        """
        Calculate exit date based on stock volatility and day of week (Jan 14, 2026 update)
        
        - High-vol stocks (NTLA, PL, etc.): D+3 minimum
        - Thu/Fri entries: Hold through Monday (weekend protection)
        - Normal stocks: D+2 default (was D+1)
        """
        # Get hold days configuration
        default_hold = getattr(self.config, 'default_hold_days', 2)
        high_vol_hold = getattr(self.config, 'high_vol_hold_days', 3)
