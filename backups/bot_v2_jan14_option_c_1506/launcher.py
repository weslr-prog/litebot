...
class BotV2Launcher:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            self.daily_summary = DailySummary(self.data_loader, self.position_tracker, self.logger)
        ...
        try:
            ...
            for symbol, alpaca_pos in alpaca_positions.items():
                ...
                if not tracked:
                    ...
                    # Calculate exit date (D+1 from actual entry)
                    exit_date_calculated = entry_date_actual + dt.timedelta(days=1)
                    ...
                    new_pos = ShortCyclePosition(
                        ...
                        exit_date=exit_date_calculated,  # D+1 from actual entry
                    ...
                    if price_diff_pct > 0.01:
                        ...
                        new_pos = ShortCyclePosition(
                            ...
                            exit_date=dt.date.today() + dt.timedelta(days=1),  # Exit D+1
    ...
    def _get_trading_phase(self) -> str:
        ...
        Returns:
            ...
            'monitoring', 'force_exit', 'postmarket', 'closed'
        """
        now = dt.datetime.now(self.tz)
        ...
        try:
            ...
            try:
                ...
                pnl_tracker.record_day(
                    ...
                    wins=sum(1 for e in summary.get('exits', []) if e.get('pnl', 0) > 0),
    ...
    def _run_entry_scan(self):
        ...
        try:
            ...
            self.enhanced_logger.log_prefilter_results(
                ...
                reasons={}  # TODO: Track rejection reasons
            )
        ...
        try:
            ...
            for position in active_positions:
                try:
                    ...
                    # Update current price - try Alpaca first, fallback to yfinance
                    current_price = self._get_realtime_price(position.symbol)
                    if not current_price:
                        # Fallback to yfinance for delayed price
                        try:
                            data = self.data_loader.get_historical_data(position.symbol, days=1)
                            if not data.empty:
                                current_price = float(data['close'].iloc[-1])
                                self.logger.debug(f"{position.symbol}: Using yfinance price ${current_price:.2f}")
                        except Exception as e:
                            self.logger.debug(f"{position.symbol}: Failed to get yfinance price: {e}")
                    ...
                    # CRITICAL FIX: If smart_exit explicitly blocks (Mon-Thu same-day hold),
                    # don't check traditional exit - respect the blocking decision
                    is_same_day = (position.entry_date == dt.date.today())
                    is_mon_thu = dt.datetime.now(pytz.UTC).weekday() in [0, 1, 2, 3]
                    is_blocked_same_day = is_same_day and is_mon_thu and not should_exit_smart
                    
                    # Check traditional exit ONLY if not blocked by same-day rule
                    should_exit_traditional = False
                    traditional_reason = None
                    if not is_blocked_same_day:
        ...
        try:
            while self.is_running:
                ...
                if phase == 'premarket':
                    ...
                    if now.weekday() == 4:  # Friday
                        print(f"⚠️  Force exit window - Friday 3:45 PM")
                        import sys
                        sys.stdout.flush()
