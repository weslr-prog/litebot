...
class SmartExitManager:
    ...
    def should_exit(self, position, current_price: float, rsi: float, 
        ...
        if is_high_vol and self.DISABLE_SMART_EXIT_HIGH_VOL:
            ...
            # DYNAMIC TRAILING STOP: Use dynamic trail based on gain size
        ...
        if profit_pct >= self.LET_WINNERS_RUN_THRESHOLD:
            # This is a runner - only exit via dynamic trailing stop
            if hasattr(position, 'highest_price') and position.highest_price:
                drawdown_from_high = (current_price - position.highest_price) / position.highest_price
                # Use dynamic trail for runners based on gain size
                dynamic_trail = self.get_dynamic_trail_pct(profit_pct)
                if drawdown_from_high <= -dynamic_trail:
                    return (True, f"RUNNER dynamic trailing ({dynamic_trail*100:.1f}% trail) from ${position.highest_price:.2f} (locked in {profit_pct*100:.1f}% gain)", current_price)
            # Let it run - no other exits
            return (False, f"🏃 RUNNER {symbol}: Letting it run (P&L: {profit_pct*100:+.1f}%, trail: {self.get_dynamic_trail_pct(profit_pct)*100:.1f}%)", current_price)
