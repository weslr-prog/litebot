...
class SmartExitManager:
    ...
    def __init__(self, config):
        ...
        # Smart exit thresholds (optimized for weekly swing - Feb 11, 2026)
        self.QUICK_PROFIT_TARGET = 0.02  # 2% - quick profit for low-confidence trades
        self.STANDARD_PROFIT_TARGET = 0.04  # 4% - standard exit target
        self.RSI_NORMALIZATION = 75  # Exit when RSI reaches overbought (was 50 - way too low)
        self.RSI_QUICK_EXIT = 80  # Quick exit if RSI > 80 (exhaustion)
        ...
        self.MAX_HOLD_HOURS = 120  # 5 trading days max hold (was 24h D+1)
    ...
    def should_exit(self, position, current_price: float, rsi: float, 
        ...
        # === MINIMUM HOLD: Wait at least 1 hour before signal-based exits ===
        if hours_held < 1.0 and profit_pct > -0.02:
        ...
        # STRATEGY 1: Quick Profit Taking (4+ hours, 2%+ gain)
        ...
        # STRATEGY 2: RSI Overbought Exit (RSI > 75 with profit)
        ...
        # STRATEGY 3: RSI Exhaustion Exit (RSI > 80 = exhaustion)
        ...
        # STRATEGY 4: Standard Profit Target (4%+ gain)
        if profit_pct >= self.STANDARD_PROFIT_TARGET:
            return (True, f"Profit target {profit_pct*100:.1f}% hit", current_price)
        
        # STRATEGY 5: Volume Exhaustion Exit (low volume + RSI > 60)
        if hours_held >= self.MIN_HOLD_HOURS:
            if volume_ratio < 0.5 and rsi > 60 and profit_pct > 0.01:
        ...
        # STRATEGY 7: Stop Loss (wider 3% for swing trades)
        if profit_pct <= -0.03:
            return (True, f"Stop loss {profit_pct*100:.1f}%", current_price)
        
        # STRATEGY 8: Dynamic Trailing Stop (after 2% profit)
        if hasattr(position, 'highest_price') and position.highest_price:
            drawdown_from_high = (current_price - position.highest_price) / position.highest_price
            if profit_pct >= self.TRAILING_STOP_TRIGGER:
                dynamic_trail = self.get_dynamic_trail_pct(profit_pct)
