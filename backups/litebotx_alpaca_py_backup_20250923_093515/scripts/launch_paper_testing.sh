...
"
        ;;
    3)
        echo "🚀 Starting ShortCycleTrader with GUI Dashboard..."
        echo "⚠️  This will run the high-ROI ShortCycleTrader with real-time GUI. Press Ctrl+C to stop."
        echo "📊 Launching integrated dashboard..."
        echo ""
        echo "🎯 ShortCycleTrader High-ROI Configuration Active..."
        python -c "
import os
...
print('🚀 ShortCycleTrader with Dashboard - Press Ctrl+C to stop')
trader = ShortCycleTrader(launch_gui=True)
trader.start_with_dashboard()
"
        ;;
    4)
        ...
        ;;
    5)
        echo "� Starting Sprint 2 ShortCycleTrader (Continuous market-hours loop)..."
        echo "⚠️  This will run the continuous scheduler with PreFilter-driven universe and detailed logs. Press Ctrl+C to stop."
        python -c "from traders.short_cycle_trader import ShortCycleTrader; ShortCycleTrader().run_continuous_cycle()"
        ;;
    *)
