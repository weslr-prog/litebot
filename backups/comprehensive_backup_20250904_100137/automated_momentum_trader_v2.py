...
class AutomatedMomentumTraderV2:
    ...
    def __init__(self, symbols: List[str] = None, alpha_vantage_key: str = None, use_enhanced_strategy: bool = True):
        ...
        # Initialize enhanced exit logic manager with refined parameters
        exit_params = ExitParameters(
            use_atr_stops=True,                    # ATR-based stops (2× ATR below entry)
            atr_stop_multiplier=2.0,               # 2× ATR stop-loss  
            fixed_stop_pct=0.025,                  # 2.5% fixed cap backup
            initial_profit_target=0.15,            # 15% minimum profit target
            atr_profit_multiplier=4.0,             # 4× ATR profit target
            scale_out_levels=[0.15, 0.25, 0.35],   # Scale out at 15%, 25%, 35%
            base_time_stop_days=12,                # 10-15 trading days (compromise at 12)
            max_time_stop_days=15,                 # Maximum 15 days
            profitable_extension_days=5,           # Extra 5 days if profitable
            trailing_stop_pct=0.08,                # 8% trailing stop
            trailing_activation_gain=0.10,         # Start trailing at 10% gain
            extended_hold_days=45,                 # 45-60 days for strong winners
            momentum_extension_threshold=0.20      # 20% gain for extended holding
        )
