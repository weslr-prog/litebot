...
def filter_universe(df):
    ...
    # Filter for stocks only (exclude ETFs, preferred stocks, etc.)
    # CS = Common Stock (good for gaps/fades)
    # REIT = Real Estate Investment Trust (too stable, exclude)
    # PFD = Preferred Stock (fixed income-like, exclude)
    # UNIT = Units/SPACs (exclude)
    # ADRC = ADR Common (can keep, foreign stocks can gap)
    ALLOWED_TYPES = {'CS', 'ADRC'}  # Common Stock and ADR Common
    ...
    # Exclude low-volatility sectors (poorly suited for Gap & Go + Fade/Short)
    # Note: Polygon doesn't provide sector data, so we filter by name keywords
    EXCLUDE_KEYWORDS = [
        # REITs (even if type=CS, some are still REITs)
        'REIT', 'Real Estate', 'Properties', 'Realty', 
        
        # Utilities (extremely stable, don't gap/extend)
        'Electric', 'Gas Co', 'Water', 'Utility', 'Power Corp',
        
        # Preferred Stocks (if any slip through)
        'Preferred', 'Series A', 'Series B', 'Series C',
        'Depositary Shares', 'Perpetual',
        
        # Trusts (business development, closed-end funds)
        'Trust', 'BDC', 'Fund Inc', 'Income Fund',
        
        # Banks/Insurance (very stable, regulated)
        'Bancorp', 'Bancshares', 'Bank N.A.', 'Bank FSB',
        'Insurance Group', 'Financial Corp'
    ]
    
    # Filter out stocks with exclude keywords in name
    exclude_mask = df['name'].str.contains('|'.join(EXCLUDE_KEYWORDS), case=False, na=False)
    excluded_count = exclude_mask.sum()
    print(f"🚫 Excluding {excluded_count} stocks (REITs, Utilities, Banks, Trusts)")
    df = df[~exclude_mask]
