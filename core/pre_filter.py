...
class PreFilter:
    ...
    def liquidity_volatility_filter(self, df, min_avg_vol=500, max_atr=5.0, min_survivors=None):
        logging.info(f"Applying TIGHTENED liquidity/volatility filter to {len(df)} assets.")
        try:
            # Filter out assets without enough lookback data
            df = self.filter_insufficient_lookback_data(df, lookback=10)

            min_avg_dollar_vol = 2000000  # MUCH TIGHTER: $2M minimum daily volume 
            ...
            eligible_symbols = latest[
                ...
                (latest['atr_10'] >= 0.01)  # Higher minimum volatility: 1.0% ATR
            ]['symbol'].tolist()
    ...
    def breakout_filter(self, df, volume_spike_min=1.2, price_breakout_min=0.005):
        """Find stocks breaking out with volume confirmation - VERY LOOSE for better throughput"""
        logging.info("Applying breakout filter - VERY LOOSE to allow more symbols through")
        ...
        # Price breakout (close vs 20-day high) - check last 2 days instead of shift(1)
        df['high_20'] = df.groupby('symbol')['high'].transform(lambda x: x.rolling(20).max().shift(1))
        df['price_breakout'] = (df['close'] - df['high_20']) / df['high_20']
        
        # Debug breakout calculations
        valid_volume_spikes = df['volume_spike'].notna().sum()
        valid_price_breakouts = df['price_breakout'].notna().sum()
        logging.info(f"Valid volume spikes: {valid_volume_spikes}, Valid price breakouts: {valid_price_breakouts}")
        
        # Check how many meet individual criteria
        volume_meets = (df['volume_spike'] >= volume_spike_min).sum()
        price_meets = (df['price_breakout'] >= price_breakout_min).sum()
        logging.info(f"Volume spike >= {volume_spike_min}: {volume_meets} rows")
        logging.info(f"Price breakout >= {price_breakout_min}: {price_meets} rows")
    ...
    def volatility_filter(self, df, min_volatility=0.010, max_volatility=0.08):
        ...
        for symbol in df['symbol'].unique()[:3]:  # Check first 3 symbols
            symbol_data = df[df['symbol'] == symbol]
            logging.info(f"  {symbol}: {len(symbol_data)} rows, date range: {symbol_data['date'].min()} to {symbol_data['date'].max()}")
        
        # Calculate percentage change and then rolling volatility
        ...
        df['volatility'] = df.groupby('symbol')['pct_change'].transform(
            lambda x: x.rolling(10, min_periods=5).std()
        )
        
        # Debug volatility
        valid_vol = df['volatility'].notna().sum()
        logging.info(f"Valid volatility values: {valid_vol} out of {len(df)}")
        
        # Log volatility statistics for debugging
        logging.info("Volatility statistics:")
        logging.info(df['volatility'].describe())
        
        # Try a simpler approach - use the last available volatility value per symbol
        latest_volatility = df.groupby('symbol')['volatility'].last()
        logging.info(f"Latest volatility per symbol (first 10):")
        logging.info(latest_volatility.head(10))
    ...
    def high_return_filter_pipeline(self, df):
        ...
        # Step 3: Price range - focus on $15-200 (sweet spot for big moves)
        df = self.price_range_filter(df, min_price=15, max_price=200)
        
        # Step 4: HIGH volatility (this is KEY - increased from 0.08 to 0.25)
        df = self.volatility_filter(df, min_volatility=0.03, max_volatility=0.30)
        ...
        # Step 6: NEW - Breakout detection (TEMPORARILY DISABLED for testing)
        # df = self.breakout_filter(df, volume_spike_min=1.5, price_breakout_min=0.02)
