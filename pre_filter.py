...
class PreFilter:
    def __init__(
        ...
        enable_gap_detection: bool = True,  # NEW: Gap-prone stock detection
    ) -> None:
        logging.info(
        ...
        # Preferred filter constants (OPTIMIZED for SMALL PORTFOLIO <$1K - Nov 24, 2025)
        # CRITICAL FIX: Relaxed dramatically to work with yfinance free tier (21 days data)
        self.MIN_AVG_DOLLAR_VOL = 500_000  # Reduced from 1M for mid-cap access
        self.MIN_AVG_VOL = 50_000  # Reduced from 100K for more opportunities
        self.MIN_PRICE = 5.0  # $5-50 range for $1K accounts (expanded for more opportunities)
        self.MAX_PRICE = 50.0  # $50 max for small account flexibility (Option 3)
        self.MIN_ATR = 0.015  # Relaxed from 3.0% to 1.5% (more candidates)
        self.MAX_ATR = 0.12  # 12% max daily range (same - avoid overnight gap risk)
        self.MIN_MOMENTUM_RETURN = 0.02  # Relaxed from 3% to 2% (more candidates)
        self.MIN_VOLUME_SURGE = 0.5  # CRITICALLY RELAXED from 1.1x to 0.5x (allow below-average volume)
        self.MIN_SURVIVORS = 6  # Accept 6+ stocks on slow days (same)
    ...
    def _rank_candidates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in [
            ...
            'rvol_5d','rvol_15d','vol_roc_1','vol_roc_3','vol_zscore_20','volume_accel','climax_flag'
        ]:
        ...
        volume_quality_score = zscore(latest['vol_zscore_20'].fillna(0.0))
        volume_accel_score = zscore(latest['volume_accel'].fillna(0.0))
        ...
        latest['score'] = (
            ...
            1.0 * vol_pref +
            0.5 * liquidity_score +
            0.4 * volume_quality_score +
            0.2 * volume_accel_score
        )
    ...
    def breakout_filter(self, df, volume_spike_min=1.2, price_breakout_min=0.005,
        ...
        # Phase 1: Volume & Momentum Core Features (daily bars)
        work.loc[:, 'rvol_5d'] = work['volume'] / work.groupby('symbol')['volume'].transform(
            lambda x: x.rolling(5, min_periods=3).mean()
        )
        work.loc[:, 'rvol_15d'] = work['volume'] / work.groupby('symbol')['volume'].transform(
            lambda x: x.rolling(15, min_periods=8).mean()
        )
        work.loc[:, 'vol_roc_1'] = work.groupby('symbol')['volume'].transform(
            lambda x: x.pct_change(periods=1, fill_method=None)
        )
        work.loc[:, 'vol_roc_3'] = work.groupby('symbol')['volume'].transform(
            lambda x: x.pct_change(periods=3, fill_method=None)
        )
        work.loc[:, 'vol_zscore_20'] = work.groupby('symbol')['volume'].transform(
            lambda x: (x - x.rolling(20, min_periods=10).mean()) / (x.rolling(20, min_periods=10).std(ddof=0) + 1e-9)
        )
        work.loc[:, 'volume_accel'] = work['vol_roc_1'] - work['vol_roc_3']

        work.loc[:, 'range_pct'] = (work['high'] - work['low']) / work['close'].replace(0, np.nan)
        work.loc[:, 'range_zscore_20'] = work.groupby('symbol')['range_pct'].transform(
            lambda x: (x - x.rolling(20, min_periods=10).mean()) / (x.rolling(20, min_periods=10).std(ddof=0) + 1e-9)
        )
        work.loc[:, 'price_change_1'] = work.groupby('symbol')['close'].transform(
            lambda x: x.pct_change(periods=1, fill_method=None)
        )
        work.loc[:, 'climax_flag'] = (
            (work['vol_zscore_20'] >= 2.0) &
            (work['range_zscore_20'] >= 1.5) &
            (work['price_change_1'].abs() >= 0.02)
        )
