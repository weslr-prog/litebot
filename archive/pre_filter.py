...
class PreFilter:
    ...
    def price_range_filter(self, df, min_price=15, max_price=300):
        ...
        # Get the latest price for each symbol to determine eligibility
        latest_prices = df.groupby('symbol')['close'].last()
        eligible_symbols = latest_prices[(latest_prices >= min_price) & (latest_prices <= max_price)].index.tolist()
        
        # Return ALL historical data for symbols that meet current price criteria
        filtered = df[df['symbol'].isin(eligible_symbols)]
    ...
    def volatility_filter(self, df, min_volatility=0.010, max_volatility=0.08):
        ...
        if not df.empty:
            ...
            logging.info("Sample data per symbol:")
            for symbol in df['symbol'].unique()[:3]:  # Show first 3 symbols
        ...
        valid_pct_changes = df['pct_change'].dropna()
        logging.info(f"Valid pct_change values: {len(valid_pct_changes)} out of {len(df)}")
        
        # Calculate volatility using 10-day rolling window
        df['volatility'] = df.groupby('symbol')['pct_change'].transform(
            lambda x: x.rolling(10, min_periods=5).std()
        )
        
        valid_volatilities = df['volatility'].dropna()
        logging.info(f"Valid volatility values: {len(valid_volatilities)} out of {len(df)}")
        
        if len(valid_volatilities) > 0:
            logging.info(f"Volatility statistics:")
            logging.info(df['volatility'].describe())
            logging.info(f"Latest volatility per symbol (first 10):")
            latest_vol = df.groupby('symbol')['volatility'].last().head(10)
            logging.info(latest_vol)
        
        # Filter based on latest volatility for each symbol
        latest_data = df.groupby('symbol').last().reset_index()
        eligible_symbols = latest_data[
            (latest_data['volatility'] >= min_volatility) & 
            (latest_data['volatility'] <= max_volatility) &
            (latest_data['volatility'].notna())
        ]['symbol'].tolist()
        
        # Return ALL historical data for symbols with valid current volatility
        filtered = df[df['symbol'].isin(eligible_symbols)]
