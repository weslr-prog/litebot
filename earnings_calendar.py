...
class EarningsCalendar:
    ...
    def should_exit_before_earnings(self, symbol: str) -> bool:
        ...
        if self.is_earnings_soon(symbol, self.exit_buffer_days):
            earnings_date = self.get_next_earnings_date(symbol)
            days_until = (earnings_date.date() - datetime.now().date()).days
            logger.warning(f"{symbol}: FORCE EXIT - Earnings in {days_until} days ({earnings_date.date()})")
            return True
        return False
    ...
    def clear_cache(self):
        ...
        self.get_next_earnings_date.cache_clear()
...
if __name__ == '__main__':
    ...
    # Test stocks with different earnings timelines
    test_symbols = ['NVDA', 'TSLA', 'AAPL', 'MSFT', 'PLTR', 'SOFI']
    ...
    for symbol in test_symbols:
        info = calendar.get_earnings_info(symbol)
        print(f"\n{symbol}:")
        print(f"  Earnings Date: {info['earnings_date']}")
        print(f"  Days Until: {info['days_until']}")
        print(f"  Status: {info['status']}")
        print(f"  Block Entry: {info['should_avoid_entry']}")
        print(f"  Force Exit: {info['should_exit']}")
