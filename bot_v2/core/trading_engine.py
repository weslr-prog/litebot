...
class ProductionTradingEngine:
    ...
    def __init__(self, config: Optional[ShortCycleConfig] = None, 
        ...
        self._kill_switch_last_day = dt.datetime.now(pytz.UTC).date()
        self._kill_switch_last_week = self._kill_switch_last_day.isocalendar()[1]
    ...
    def run_daily_cycle(self):
        ...
        try:
            self._reset_kill_switches_if_needed()
    ...
    def _reset_kill_switches_if_needed(self):
        """Auto-reset kill switches on new day/week so stale states do not block trading."""
        now_day = dt.datetime.now(pytz.UTC).date()
        now_week = now_day.isocalendar()[1]

        if now_day != self._kill_switch_last_day:
            if self.kill_switches.get("daily_loss_exceeded"):
                self.logger.info("♻️ Reset daily kill switch for new trading day")
            self.kill_switches["daily_loss_exceeded"] = False
            self._kill_switch_last_day = now_day

        if now_week != self._kill_switch_last_week:
            if self.kill_switches.get("weekly_loss_exceeded"):
                self.logger.info("♻️ Reset weekly kill switch for new trading week")
            self.kill_switches["weekly_loss_exceeded"] = False
            self._kill_switch_last_week = now_week

        if self.kill_switches.get("system_error"):
            self.logger.info("♻️ Clearing stale system_error kill switch at cycle start")
            self.kill_switches["system_error"] = False
    ...
    def _process_existing_positions(self):
        ...
        # Phase 1: Check for earnings/time-stop exits
        strategic_exits = self.exit_manager.process_strategic_d1_exits(
            positions, self.data_loader
        )
        
        # Phase 2: Other exit conditions (trailing stops, stop loss, smart exits)
        other_exits = 0
        for position in positions:
            if position.status != PositionStatus.ENTERED:
                continue
            
            # No same-day blocks - all positions eligible for exit based on signals
            # Minimum 1-hour hold is enforced in position.is_d1_eligible()
    ...
    def _generate_and_execute_new_positions(self):
        ...
        try:
            ...
            regime = (regime_info.get('regime') or "NEUTRAL").upper()
            ...
            # Regime-based gating and dynamic quality filter
            allowed_by_regime = {
                "BULL": {"momentum", "gap_and_go"},
                "BEAR": {"fade_short", "mean_reversion"},
                "NEUTRAL": {"momentum", "gap_and_go", "fade_short", "mean_reversion"}
            }
            min_conf_by_regime = {
                "BULL": 0.80,
                "BEAR": 0.80,
            ...
            allowed_strategies = allowed_by_regime.get(regime, allowed_by_regime["NEUTRAL"])
            min_conf = min_conf_by_regime.get(regime, 0.80)
            if regime_conf >= 0.75:
                min_conf = min_conf + 0.05
            
            filtered = []
            for signal in signals:
                strategy = (signal.features_used or {}).get("strategy", "").lower()
                if strategy and strategy not in allowed_strategies:
                    self.logger.info(
                        f"🧭 Regime gate: {signal.symbol} skipped (strategy={strategy}, regime={regime})"
                    )
                    continue
                if signal.confidence < min_conf:
                    self.logger.info(
                        f"🧹 Quality gate: {signal.symbol} skipped (conf={signal.confidence:.2f} < {min_conf:.2f})"
                    )
                    continue
                filtered.append(signal)
            
            if not filtered:
                self.logger.info("📭 No signals after regime/quality gating")
                return
            signals = filtered
    ...
    def _calculate_exit_date(self, signal, symbol_data: pd.DataFrame) -> dt.date:
        """Calculate exit date based on momentum: 5-day default, longer for strong momentum"""
        today = dt.date.today()
        ...
        try:
            if symbol_data is not None and len(symbol_data) >= 5:
                ...
                # Strong momentum: hold longer (D+5)
                if momentum >= self.config.strong_momentum_threshold:
                    hold_days = 5
                    self.logger.info(f"📈 {signal.symbol}: STRONG momentum {momentum:.1%} → D+5 exit")
                # Good momentum: D+3
                elif momentum >= self.config.momentum_hold_threshold:
                    hold_days = 3
                    self.logger.info(f"📊 {signal.symbol}: Good momentum {momentum:.1%} → D+3 exit")
                # Standard: D+2
                else:
                    hold_days = 2
                    self.logger.info(f"📋 {signal.symbol}: Standard momentum {momentum:.1%} → D+2 exit")
            else:
                hold_days = 3  # Default to D+3 if no data
                self.logger.info(f"📋 {signal.symbol}: No momentum data → D+3 exit (default)")
        except Exception as e:
            self.logger.warning(f"⚠️ {signal.symbol}: Error calculating momentum: {e}, using D+3")
            hold_days = 3
