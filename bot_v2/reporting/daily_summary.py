...
class DailySummary:
    ...
    def __init__(self, data_loader, position_tracker, logger=None, trading_engine=None):
        self.data_loader = data_loader
        self.position_tracker = position_tracker
        self.logger = logger or logging.getLogger(__name__)
        self.trading_engine = trading_engine  # For real Alpaca equity
    ...
    def generate_summary(self, session_data: Dict) -> Dict:
        ...
        try:
            ...
            def _is_entered(p):
                if PositionStatus and isinstance(p.status, PositionStatus):
                    return p.status == PositionStatus.ENTERED
                return str(p.status).lower() in {"entered", "open"}
            
            def _is_exited(p):
                if PositionStatus and isinstance(p.status, PositionStatus):
                    return p.status == PositionStatus.EXITED
                return str(p.status).lower() in {"exited", "closed"}
            ...
            def _to_date(value):
                ...
                if isinstance(value, str):
                    try:
                        ...
                        pass
                    try:
                        # Try parsing ISO timestamp with timezone info
                        # Remove timezone info and parse as datetime, then get date
                        # e.g., "2026-02-05T15:35:44.092058+00:00" → "2026-02-05"
                        if 'T' in value:
                            # Split at + or Z to remove timezone
                            ts_str = value.split('+')[0].split('Z')[0]
            ...
            active_positions = [p for p in positions if _is_entered(p)]
            closed_today = []
            for p in positions:
                if not _is_exited(p):
                    continue
                exit_dt = _to_date(getattr(p, 'exit_timestamp', None))
                if exit_dt is None:
                    exit_dt = _to_date(getattr(p, 'exit_date', None))
                if exit_dt == now.date():
                    closed_today.append(p)
            
            # Calculate P&L
            realized_pnl = sum(p.realized_pnl for p in closed_today if p.realized_pnl is not None)
            unrealized_pnl = sum(self._calculate_unrealized_pnl(p) for p in active_positions)
            total_pnl = realized_pnl + unrealized_pnl
            
            # Get deployed capital
            deployed_capital = sum(p.entry_price * p.position_size_shares for p in active_positions)
            ...
            summary = {
                ...
                'rejection_samples': session_data.get('rejection_samples', {}),
                'entries': session_data.get('entries_executed', []),
                'exits': [self._format_exit(p) for p in closed_today],
                'exit_reason_distribution': session_data.get('exit_reasons', self._build_exit_reason_distribution(closed_today)),
    ...
    def _calculate_unrealized_pnl(self, position) -> float:
        ...
        try:
            ...
            if position.symbol in bars and len(bars[position.symbol]) > 0:
                ...
                return (current_price - position.entry_price) * position.position_size_shares
            return 0.0
        except:
            return 0.0
    ...
    def _get_week_to_date_stats(self, now: dt.datetime) -> Dict:
        ...
        try:
            ...
            # Convert entry_date to datetime if it's a date object
            def to_datetime(d):
                if isinstance(d, dt.date) and not isinstance(d, dt.datetime):
                    return dt.datetime.combine(d, dt.time.min).replace(tzinfo=self.tz)
                return d
            week_positions = [p for p in positions if to_datetime(p.entry_date) >= monday]
            
            # Calculate stats
            closed_positions = [p for p in week_positions if _is_exited(p)]
            winners = [p for p in closed_positions if p.realized_pnl is not None and p.realized_pnl > 0]
            losers = [p for p in closed_positions if p.realized_pnl is not None and p.realized_pnl <= 0]
            
            win_rate = (len(winners) / len(closed_positions) * 100) if closed_positions else 0
            total_pnl = sum(p.realized_pnl for p in closed_positions if p.realized_pnl is not None)
            
            # Get starting portfolio value from Alpaca (source of truth)
            starting_value = None
            if self.trading_engine:
                try:
                    account_info = self.trading_engine.get_account_info()
                    if account_info and 'portfolio_value' in account_info:
                        starting_value = float(account_info['portfolio_value'])
                except Exception as e:
                    self.logger.debug(f"Could not get Alpaca equity: {e}")
            if starting_value is None:
                starting_value = 1000.0  # Fallback only if Alpaca unavailable
            ...
            return {
                ...
                'portfolio_value': starting_value if 'starting_value' in dir() else 0,
                'weekly_return': 0,
                'monday_date': 'N/A'
            }
    ...
    def _format_position(self, position) -> Dict:
        ...
        try:
            ...
            unrealized_pct = (unrealized_pnl / (position.entry_price * position.position_size_shares)) * 100
    ...
    def _format_exit(self, position) -> Dict:
        ...
        return {
            ...
            'realized_pct': (position.realized_pnl / (position.entry_price * position.position_size_shares)) * 100 if position.entry_price else 0,
    ...
    def _save_daily_stats(self, summary: Dict):
        ...
        try:
            ...
            # Replace or append today's stats (dedupe by date)
            date_iso = summary.get('date_iso')
            if not date_iso:
                date_iso = dt.datetime.strptime(summary['date'], '%A, %B %d, %Y').strftime('%Y-%m-%d')
            ...
            daily_record = {
                ...
                'signals_generated': summary['activity']['signals_generated'],
                'candidates_reviewed': summary['activity']['candidates_reviewed'],
                'rejection_total': int(sum(summary.get('rejections', {}).values())) if summary.get('rejections') else 0,
                'top_rejections': dict(sorted(summary.get('rejections', {}).items(), key=lambda x: x[1], reverse=True)[:5]),
                'exit_reason_distribution': summary.get('exit_reason_distribution', {})
            }

            replaced = False
            for idx, record in enumerate(all_stats):
                existing_iso = record.get('date_iso')
                if not existing_iso and record.get('date'):
                    try:
                        existing_iso = dt.datetime.strptime(record['date'], '%A, %B %d, %Y').strftime('%Y-%m-%d')
                    except Exception:
                        existing_iso = None
                if existing_iso == date_iso:
                    all_stats[idx] = daily_record
                    replaced = True
                    break
    ...
    def _build_exit_reason_distribution(self, closed_positions: List) -> Dict[str, int]:
        """Build normalized exit reason distribution from closed positions."""
        distribution: Dict[str, int] = {}
        for position in closed_positions:
            tag = self._normalize_exit_reason(getattr(position, 'exit_reason', None))
            distribution[tag] = distribution.get(tag, 0) + 1
        return distribution

    def _normalize_exit_reason(self, reason: str) -> str:
        """Normalize free-form exit reason text to stable report tags."""
    ...
    def _print_detailed_sections(self, summary: Dict):
        ...
        if summary['entries']:
            ...
            for entry in summary['entries']:
                ...
                if 'entry_price' in entry and 'entry_time' in entry:
                    print(f"    • Entry: ${entry['entry_price']:.2f} @ {entry['entry_time']}")
                elif 'entry_price' in entry:
